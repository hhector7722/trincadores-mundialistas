...
export function collectExtendedLayoutMetrics(pathname: string): ExtendedLayoutMetrics {
  ...
  return {
    ...
    gapBelowShell: Math.round(visualBottom - (shellRect?.bottom ?? 0)),
    ...
    docScrollTop: Math.round(docScrollTop),
    docCanScroll: document.documentElement.scrollHeight > window.innerHeight + 1,
