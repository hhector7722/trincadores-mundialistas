...
export function HomeChampionTeamValue({
  ...
  return (
    <span className="inline-flex max-w-full flex-wrap items-center justify-center gap-x-0.5 gap-y-0">
      <TeamFlagBadge name={team} size="xxs" loading="eager" className="shrink-0" />
      <span
        className={cn(
          "line-clamp-2 break-words text-center text-[clamp(8px,2.4cqw,10px)] font-medium leading-tight",
          abbrClassName
        )}
      >
        {teamAbbr(team)}
...
export function HomeFinalistsTeamValue({ teamA, teamB }: { teamA: string; teamB: string }) {
  return (
    <span className="inline-flex max-w-full flex-wrap items-center justify-center gap-x-0.5 gap-y-0 text-[clamp(8px,2.4cqw,10px)] font-medium leading-tight text-[#CCFF00]">
