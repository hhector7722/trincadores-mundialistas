...
export function GroupStandingsTable({
      ...
      <>
        ...
        <span
          className={cn(
            ...
            isGrid
              ? "whitespace-nowrap text-left text-[7px] font-semibold uppercase tracking-wide"
  ...
  return (
    <div
      className={cn(
        isGrid ? "flex min-h-0 flex-1 flex-col overflow-visible px-0.5 pb-0.5" : "overflow-x-auto",
    ...
    >
      <table
        className={cn(
          ...
          isGrid
            ? "w-full table-fixed text-[7px] leading-none"
            : isCompact
      ...
      >
        ...
        <tbody>
          {group.teams.map((row, index) => (
            ...
            >
              <td className={cn(isGrid ? "py-px pr-0.5 align-middle" : isCompact ? "py-0.5 pr-1" : "py-2 pr-2")}>
                {renderTeamCell(row)}
              </td>
              {statColumns.map((col) => (
                <td
