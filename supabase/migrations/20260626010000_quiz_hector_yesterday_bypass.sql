-- RPC para hector: jugar quiz de ayer saltando la validacion de ventana cerrada.

drop function if exists public.start_quiz_attempt_yesterday(uuid);

create or replace function public.start_quiz_attempt_yesterday(p_quiz_id uuid)
returns jsonb
language plpgsql
security definer
...
grant execute on function public.start_quiz_attempt_yesterday(uuid) to authenticated;
